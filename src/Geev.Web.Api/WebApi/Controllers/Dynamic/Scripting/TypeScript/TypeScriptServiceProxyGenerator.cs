...
using Geev.Web;
using Geev.WebApi.Controllers.Dynamic.Scripting.Angular;

namespace Geev.WebApi.Controllers.Dynamic.Scripting.TypeScript
{
    internal class TypeScriptServiceProxyGenerator : ITransientDependency
    {
        ...
        public string Generate(DynamicApiControllerInfo controllerInfo, string servicePrefix)
        {
            ...
            foreach (var methodInfo in _controllerInfo.Actions.Values)
            {
                ...
                if (returnType == "void")
                {
                    script.AppendLine(string.Format("           public {0} = function ({1}): geev.IPromise ",
                    ...
                    script.AppendLine("                        geev: true,");
                    script.AppendLine("                        url: geev.appPath + '" + ActionScriptingHelper.GenerateUrlWithParameters(_controllerInfo, methodInfo) + "',");
                ...
                {
                    script.AppendLine(string.Format("           public {0} = function ({1}): geev.IGenericPromise<{2}> ", methodInfo.ActionName.ToCamelCase(), 
            ...
            script.AppendLine("angular.module('geev').service('geev.services." + _controllerInfo.ServiceName.Replace("/", ".") + "', geev.services." + _controllerInfo.ServiceName.Replace("/", ".")+");");

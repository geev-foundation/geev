...
namespace Geev.WebApi.Controllers.Dynamic.Scripting
{
    internal static class ActionScriptingHelper
    {
        public static string GenerateUrlWithParameters(DynamicApiControllerInfo controllerInfo, DynamicApiActionInfo actionInfo)
        {
            ...
            return baseUrl + $"' + geev.utils.buildQueryString([{qsBuilderParams}]) + '";
        }
        ...
        public static string GenerateBody(DynamicApiActionInfo actionInfo)
        {
            ...
            {
                throw new GeevException("Only one complex type allowed as argument to a web api controller action. But " + actionInfo.ActionName + " contains more than one!");
            }

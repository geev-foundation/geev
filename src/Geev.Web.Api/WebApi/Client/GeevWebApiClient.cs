...
{
    public class GeevWebApiClient : ITransientDependency, IGeevWebApiClient
    {
        ...
        static GeevWebApiClient()
        {
            DefaultTimeout = TimeSpan.FromSeconds(90);
        }

        public GeevWebApiClient()
        {
                ...
                using (var client = new HttpClient(handler))
                {
                    ...
                    using (var requestContent = new StringContent(Object2JsonString(input), Encoding.UTF8, "application/json"))
                    {
                        ...
                        using (var response = await client.PostAsync(url, requestContent))
                        {
                            ...
                            {
                                throw new GeevException("Could not made request to " + url + "! StatusCode: " + response.StatusCode + ", ReasonPhrase: " + response.ReasonPhrase);
                            }

                            var ajaxResponse = JsonString2Object<AjaxResponse<TResult>>(await response.Content.ReadAsStringAsync());
                            if (!ajaxResponse.Success)
                            {
                                throw new GeevRemoteCallException(ajaxResponse.Error);
                            }

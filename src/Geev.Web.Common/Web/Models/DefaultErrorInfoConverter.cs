...
namespace Geev.Web.Models
{
    ...
    {
        private readonly IGeevWebCommonModuleConfiguration _configuration;
        ...
        public DefaultErrorInfoConverter(
            IGeevWebCommonModuleConfiguration configuration,
        ...
        private ErrorInfo CreateErrorInfoWithoutCode(Exception exception)
        {
            ...
            if (exception is AggregateException && exception.InnerException != null)
            {
                ...
                if (aggException.InnerException is UserFriendlyException ||
                    aggException.InnerException is GeevValidationException)
                {
            ...
            if (exception is GeevValidationException)
            {
                ...
                {
                    ValidationErrors = GetValidationErrorInfos(exception as GeevValidationException),
                    Details = GetValidationErrorNarrative(exception as GeevValidationException)
                };
            }
            ...
            if (exception is Geev.Authorization.GeevAuthorizationException)
            {
                var authorizationException = exception as Geev.Authorization.GeevAuthorizationException;
        ...
        private ErrorInfo CreateDetailedErrorInfoFromException(Exception exception)
        {
            ...
            if (exception is GeevValidationException)
            {
                errorInfo.ValidationErrors = GetValidationErrorInfos(exception as GeevValidationException);
            }
        ...
        private void AddExceptionToDetails(Exception exception, StringBuilder detailBuilder)
        {
            ...
            //Additional info for GeevValidationException
            if (exception is GeevValidationException)
            {
        ...
        private ValidationErrorInfo[] GetValidationErrorInfos(GeevValidationException validationException)
        {
        ...
        private string L(string name)
        {
            try
            {
                return _localizationManager.GetString(GeevWebConsts.LocalizaionSourceName, name);
            }
            catch (Exception)
            {
                return name;

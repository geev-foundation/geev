...
namespace Geev.Web.Navigation
{
    internal class NavigationScriptManager : INavigationScriptManager, ITransientDependency
    {
        ...
        {
            var userMenus = await _userNavigationManager.GetMenusAsync(GeevSession.ToUserIdentifier());

            var script = new StringBuilder();
            script.AppendLine("(function() {");

            script.AppendLine("    geev.nav = {};");
            script.AppendLine("    geev.nav.menus = {");

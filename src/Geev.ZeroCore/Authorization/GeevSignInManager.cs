...
{
    public class GeevSignInManager<TTenant, TRole, TUser> : SignInManager<TUser>, ITransientDependency
        ...
        where TUser : GeevUser<TUser>
    {
        ...
        public GeevSignInManager(
            GeevUserManager<TRole, TUser> userManager,
            IHttpContextAccessor contextAccessor,
            GeevUserClaimsPrincipalFactory<TUser, TRole> claimsFactory,
        ...
        public virtual async Task<SignInResult> SignInOrTwoFactorAsync(GeevLoginResult<TTenant, TUser> loginResult, bool isPersistent, bool? rememberBrowser = null, string loginProvider = null, bool bypassTwoFactor = false)
        {
            ...
            {
                await UserManager.As<GeevUserManager<TRole, TUser>>().InitializeOptionsAsync(loginResult.Tenant?.Id);

                if (!bypassTwoFactor && IsTrue(GeevZeroSettingNames.UserManagement.TwoFactorLogin.IsEnabled, loginResult.Tenant?.Id))
                {
        ...
        protected virtual ClaimsPrincipal StoreTwoFactorInfo(TUser user, string loginProvider)
        {
            ...
            {
                identity.AddClaim(new Claim(GeevClaimTypes.TenantId, user.TenantId.Value.ToString()));
            }
        ...
        public async Task<int?> GetVerifiedTenantIdAsync()
        {
            ...
            return GeevZeroClaimsIdentityHelper.GetTenantId(result.Principal);
        }
        ...
        public override async Task<bool> IsTwoFactorClientRememberedAsync(TUser user)
        {
            ...
            return result?.Principal != null &&
                   ...
                   GeevZeroClaimsIdentityHelper.GetTenantId(result.Principal) == user.TenantId;
        }
        ...
        public override async Task RememberTwoFactorClientAsync(TUser user)
        {
            ...
            if (user.TenantId.HasValue)
            {
                rememberBrowserIdentity.AddClaim(new Claim(GeevClaimTypes.TenantId, user.TenantId.Value.ToString()));
            }

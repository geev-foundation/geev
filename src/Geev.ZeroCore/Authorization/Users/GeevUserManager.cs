...
{
    public class GeevUserManager<TRole, TUser> : UserManager<TUser>, IDomainService
        ...
        where TUser : GeevUser<TUser>
    {
        protected IUserPermissionStore<TUser> UserPermissionStore
        {
            get
            {
                ...
                {
                    throw new GeevException("Store is not IUserPermissionStore");
                }
        ...
        protected GeevRoleManager<TRole, TUser> RoleManager { get; }

        protected GeevUserStore<TRole, TUser> GeevUserStore { get; }
        ...
        public GeevUserManager(
            GeevRoleManager<TRole, TUser> roleManager,
            GeevUserStore<TRole, TUser> userStore,
                ...
                logger)
        {
            ...
            GeevUserStore = userStore;
            RoleManager = roleManager;
            LocalizationManager = NullLocalizationManager.Instance;
            LocalizationSourceName = GeevZeroConsts.LocalizationSourceName;
        }
        ...
        {
            return GeevUserStore.FindByNameOrEmailAsync(userNameOrEmailAddress);
        }

        public virtual Task<List<TUser>> FindAllAsync(UserLoginInfo login)
        {
            return GeevUserStore.FindAllAsync(login);
        }

        public virtual Task<TUser> FindAsync(int? tenantId, UserLoginInfo login)
        {
            return GeevUserStore.FindAsync(tenantId, login);
        }

        public virtual Task<TUser> FindByNameOrEmailAsync(int? tenantId, string userNameOrEmailAddress)
        {
            return GeevUserStore.FindByNameOrEmailAsync(tenantId, userNameOrEmailAddress);
        }
        ...
        /// <exception cref="GeevException">Throws exception if no user found with given id</exception>
        public virtual async Task<TUser> GetUserByIdAsync(long userId)
        {
            var user = await FindByIdAsync(userId.ToString());
            if (user == null)
            {
                throw new GeevException("There is no user with id: " + userId);
            }
        ...
        public override async Task<IdentityResult> UpdateAsync(TUser user)
        {
            ...
            if (user.UserName != GeevUserBase.AdminUserName)
            {
                if ((await GetOldUserNameAsync(user.Id)) == GeevUserBase.AdminUserName)
                {
                    throw new UserFriendlyException(string.Format(L("CanNotRenameAdminUser"), GeevUserBase.AdminUserName));
                }
            }
        ...
        public override async Task<IdentityResult> DeleteAsync(TUser user)
        {
            if (user.UserName == GeevUserBase.AdminUserName)
            {
                throw new UserFriendlyException(string.Format(L("CanNotDeleteAdminUser"), GeevUserBase.AdminUserName));
            }
        ...
        public virtual async Task<IdentityResult> ChangePasswordAsync(TUser user, string newPassword)
        {
            ...
            await GeevUserStore.SetPasswordHashAsync(user, PasswordHasher.HashPassword(user, newPassword));
        ...
        {
            await GeevUserStore.UserRepository.EnsureCollectionLoadedAsync(user, u => u.Roles);
        ...
        private async Task CheckMaxUserOrganizationUnitMembershipCountAsync(int? tenantId, int requestedCount)
        {
            ...
            {
                throw new GeevException($"Can not set more than {maxCount} organization unit for a user!");
            }
        }
        ...
        public virtual async Task InitializeOptionsAsync(int? tenantId)
        {
            ...
            //Lockout
            Options.Lockout.AllowedForNewUsers = await IsTrueAsync(GeevZeroSettingNames.UserManagement.UserLockOut.IsEnabled, tenantId);
            Options.Lockout.DefaultLockoutTimeSpan = TimeSpan.FromSeconds(await GetSettingValueAsync<int>(GeevZeroSettingNames.UserManagement.UserLockOut.DefaultAccountLockoutSeconds, tenantId));
            Options.Lockout.MaxFailedAccessAttempts = await GetSettingValueAsync<int>(GeevZeroSettingNames.UserManagement.UserLockOut.MaxFailedAccessAttemptsBeforeLockout, tenantId);

            //Password complexity
            Options.Password.RequireDigit = await GetSettingValueAsync<bool>(GeevZeroSettingNames.UserManagement.PasswordComplexity.RequireDigit, tenantId);
            Options.Password.RequireLowercase = await GetSettingValueAsync<bool>(GeevZeroSettingNames.UserManagement.PasswordComplexity.RequireLowercase, tenantId);
            Options.Password.RequireNonAlphanumeric = await GetSettingValueAsync<bool>(GeevZeroSettingNames.UserManagement.PasswordComplexity.RequireNonAlphanumeric, tenantId);
            Options.Password.RequireUppercase = await GetSettingValueAsync<bool>(GeevZeroSettingNames.UserManagement.PasswordComplexity.RequireUppercase, tenantId);
            Options.Password.RequiredLength = await GetSettingValueAsync<int>(GeevZeroSettingNames.UserManagement.PasswordComplexity.RequiredLength, tenantId);
        }

        protected virtual Task<string> GetOldUserNameAsync(long userId)
        {
            return GeevUserStore.GetUserNameFromDatabaseAsync(userId);
        }
        ...
        public override async Task<IList<string>> GetValidTwoFactorProvidersAsync(TUser user)
        {
            ...
            {
                if (provider == "Email" &&
                    !await IsTrueAsync(GeevZeroSettingNames.UserManagement.TwoFactorLogin.IsEmailProviderEnabled, user.TenantId))
                {
                    continue;
                }

                if (provider == "Phone" &&
                    !await IsTrueAsync(GeevZeroSettingNames.UserManagement.TwoFactorLogin.IsSmsProviderEnabled, user.TenantId))
                {
                    continue;
                }
        ...
        private MultiTenancySides GetCurrentMultiTenancySide()
        {
            ...
            return GeevSession.MultiTenancySide;
        }
        ...
        {
            await GeevUserStore.AddTokenValidityKeyAsync(user, tokenValidityKey, expireDate, cancellationToken);
        }
        ...
        public virtual async Task<bool> IsTokenValidityKeyValidAsync(
            ...
            CancellationToken cancellationToken = default(CancellationToken))
        {
            return await GeevUserStore.IsTokenValidityKeyValidAsync(user, tokenValidityKey, cancellationToken);
        }
        ...
        public virtual async Task RemoveTokenValidityKeyAsync(
            ...
            CancellationToken cancellationToken = default(CancellationToken))
        {
            await GeevUserStore.RemoveTokenValidityKeyAsync(user, tokenValidityKey, cancellationToken);
        }
    }
}
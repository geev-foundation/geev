...
namespace Geev.Authorization.Roles
{
    ...
    /// </summary>
    public abstract class GeevRoleManager<TRole, TUser>
        ...
        where TUser : GeevUser<TUser>
    {
        ...
        private IRolePermissionStore<TRole> RolePermissionStore
        {
            get
            {
                ...
                {
                    throw new GeevException("Store is not IRolePermissionStore");
                }
        ...
        protected GeevRoleStore<TRole, TUser> GeevStore { get; private set; }
        ...
        /// Constructor.
        /// </summary>
        protected GeevRoleManager(
            GeevRoleStore<TRole, TUser> store,
        ...
        public async override Task<IdentityResult> DeleteAsync(TRole role)
        {
            if (role.IsStatic)
            {
                return GeevIdentityResult.Failed(string.Format(L("CanNotDeleteStaticRole"), role.Name));
            }
        ...
        /// <exception cref="GeevException">Throws exception if no role with given id</exception>
        public virtual async Task<TRole> GetRoleByIdAsync(int roleId)
        {
            var role = await FindByIdAsync(roleId);
            if (role == null)
            {
                throw new GeevException("There is no role with id: " + roleId);
            }
        ...
        /// <param name="roleName">Role name</param>
        /// <returns>Role</returns>
        /// <exception cref="GeevException">Throws exception if no role with given roleName</exception>
        public virtual async Task<TRole> GetRoleByNameAsync(string roleName)
        {
            var role = await FindByNameAsync(roleName);
            if (role == null)
            {
                throw new GeevException("There is no role with name: " + roleName);
            }
        ...
        public virtual async Task<IdentityResult> CheckDuplicateRoleNameAsync(int? expectedRoleId, string name, string displayName)
        {
            ...
            {
                return GeevIdentityResult.Failed(string.Format(L("RoleNameIsAlreadyTaken"), name));
            }

            role = await FindByDisplayNameAsync(displayName);
            if (role != null && role.Id != expectedRoleId)
            {
                return GeevIdentityResult.Failed(string.Format(L("RoleDisplayNameIsAlreadyTaken"), displayName));
            }
        ...
        {
            return GeevStore.FindByDisplayNameAsync(displayName);
        }
        ...
        private async Task<RolePermissionCacheItem> GetRolePermissionCacheItemAsync(int roleId)
        {
            ...
            return await CacheManager.GetRolePermissionCache().GetAsync(cacheKey, async () =>
            {
                ...
                if (role == null)
                {
                    throw new GeevException("There is no role with given id: " + roleId);
                }

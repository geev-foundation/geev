...
{
    public abstract class GeevLogInManager<TTenant, TRole, TUser> : ITransientDependency
        ...
        where TUser : GeevUser<TUser>
    {
        ...
        protected GeevUserManager<TRole, TUser> UserManager { get; }
        ...
        [UnitOfWork]
        public virtual async Task<GeevLoginResult<TTenant, TUser>> LoginAsync(UserLoginInfo login, string tenancyName = null)
        {
        ...
        protected virtual async Task<GeevLoginResult<TTenant, TUser>> LoginAsyncInternal(UserLoginInfo login, string tenancyName)
        {
            ...
            else if (!string.IsNullOrWhiteSpace(tenancyName))
            {
                ...
                if (tenant == null)
                {
                    return new GeevLoginResult<TTenant, TUser>(GeevLoginResultType.InvalidTenancyName);
                }

                if (!tenant.IsActive)
                {
                    return new GeevLoginResult<TTenant, TUser>(GeevLoginResultType.TenantIsNotActive, tenant);
                }
            }
            ...
            {
                var user = await UserManager.GeevStore.FindAsync(tenantId, login);
                if (user == null)
                {
                    return new GeevLoginResult<TTenant, TUser>(GeevLoginResultType.UnknownExternalLogin, tenant);
                }
        ...
        [UnitOfWork]
        public virtual async Task<GeevLoginResult<TTenant, TUser>> LoginAsync(string userNameOrEmailAddress, string plainPassword, string tenancyName = null, bool shouldLockout = true)
        {
        ...
        protected virtual async Task<GeevLoginResult<TTenant, TUser>> LoginAsyncInternal(string userNameOrEmailAddress, string plainPassword, string tenancyName, bool shouldLockout)
        {
            ...
            using (UnitOfWorkManager.Current.SetTenantId(tenantId))
            {
                //TryLoginFromExternalAuthenticationSources method may create the user, that's why we are calling it before GeevStore.FindByNameOrEmailAsync
                ...
                var user = await UserManager.GeevStore.FindByNameOrEmailAsync(tenantId, userNameOrEmailAddress);
                if (user == null)
                {
                    return new GeevLoginResult<TTenant, TUser>(GeevLoginResultType.InvalidUserNameOrEmailAddress, tenant);
                }

                if (await UserManager.IsLockedOutAsync(user.Id))
                {
                    return new GeevLoginResult<TTenant, TUser>(GeevLoginResultType.LockedOut, tenant, user);
                }
        ...
        protected virtual async Task<GeevLoginResult<TTenant, TUser>> GetFailedPasswordValidationAsLoginResultAsync(TUser user, TTenant tenant = null, bool shouldLockout = false)
        {
            if (shouldLockout)
            {
            ...
            return new GeevLoginResult<TTenant, TUser>(GeevLoginResultType.InvalidPassword, tenant, user);
        }

        protected virtual async Task<GeevLoginResult<TTenant, TUser>> GetSuccessRehashNeededAsLoginResultAsync(TUser user, TTenant tenant = null, bool shouldLockout = false)
        {
            return await GetFailedPasswordValidationAsLoginResultAsync(user, tenant, shouldLockout);
        }

        protected virtual async Task<GeevLoginResult<TTenant, TUser>> CreateLoginResultAsync(TUser user, TTenant tenant = null)
        {
            if (!user.IsActive)
            {
                return new GeevLoginResult<TTenant, TUser>(GeevLoginResultType.UserIsNotActive);
            }

            if (await IsEmailConfirmationRequiredForLoginAsync(user.TenantId) && !user.IsEmailConfirmed)
            {
                return new GeevLoginResult<TTenant, TUser>(GeevLoginResultType.UserEmailIsNotConfirmed);
            }

            return new GeevLoginResult<TTenant, TUser>(
                tenant,
                user,
        ...
        protected virtual async Task SaveLoginAttempt(GeevLoginResult<TTenant, TUser> loginResult, string tenancyName, string userNameOrEmailAddress)
        {
                    ...
                    if (await source.Object.TryAuthenticateAsync(userNameOrEmailAddress, plainPassword, tenant))
                    {
                        ...
                        using (UnitOfWorkManager.Current.SetTenantId(tenantId))
                        {
                            ...
                            if (user == null)
                            {
                                ...
                                await UserManager.GeevStore.CreateAsync(user);
                            }
                            else
                            {
                                ...
                                await UserManager.GeevStore.UpdateAsync(user);
                            }
        ...
        {
            var tenant = await TenantRepository.FirstOrDefaultAsync(t => t.TenancyName == GeevTenant<TUser>.DefaultTenantName);
            if (tenant == null)
            {
                throw new GeevException("There should be a 'Default' tenant if multi-tenancy is disabled!");
            }
        ...
        protected virtual async Task<bool> IsEmailConfirmationRequiredForLoginAsync(int? tenantId)
        {
            ...
            {
                return await SettingManager.GetSettingValueForTenantAsync<bool>(GeevZeroSettingNames.UserManagement.IsEmailConfirmationRequiredForLogin, tenantId.Value);
            }

            return await SettingManager.GetSettingValueForApplicationAsync<bool>(GeevZeroSettingNames.UserManagement.IsEmailConfirmationRequiredForLogin);
        }
    }
}

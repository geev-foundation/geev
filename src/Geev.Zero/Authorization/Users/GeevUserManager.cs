...
namespace Geev.Authorization.Users
{
    ...
    /// </summary>
    public abstract class GeevUserManager<TRole, TUser>
        ...
        where TUser : GeevUser<TUser>
    {
        ...
        public GeevUserStore<TRole, TUser> GeevStore { get; }
        ...
        {
            return await GeevStore.FindByNameOrEmailAsync(userNameOrEmailAddress);
        }

        public virtual Task<List<TUser>> FindAllAsync(UserLoginInfo login)
        {
            return GeevStore.FindAllAsync(login);
        }
        ...
        public async override Task<ClaimsIdentity> CreateIdentityAsync(TUser user, string authenticationType)
        {
            ...
            {
                identity.AddClaim(new Claim(GeevClaimTypes.TenantId, user.TenantId.Value.ToString(CultureInfo.InvariantCulture)));
            }
        ...
        public async override Task<IdentityResult> UpdateAsync(TUser user)
        {
            ...
            if (user.UserName != GeevUser<TUser>.AdminUserName)
            {
                if ((await GetOldUserNameAsync(user.Id)) == GeevUser<TUser>.AdminUserName)
                {
                    return GeevIdentityResult.Failed(string.Format(L("CanNotRenameAdminUser"), GeevUser<TUser>.AdminUserName));
                }
            }
        ...
        public async override Task<IdentityResult> DeleteAsync(TUser user)
        {
            if (user.UserName == GeevUser<TUser>.AdminUserName)
            {
                return GeevIdentityResult.Failed(string.Format(L("CanNotDeleteAdminUser"), GeevUser<TUser>.AdminUserName));
            }
        ...
        public virtual async Task<IdentityResult> ChangePasswordAsync(TUser user, string newPassword)
        {
            ...
            await GeevStore.SetPasswordHashAsync(user, PasswordHasher.HashPassword(newPassword));
        ...
        public virtual async Task<IdentityResult> CheckDuplicateUsernameOrEmailAddressAsync(long? expectedUserId, string userName, string emailAddress)
        {
            ...
            {
                return GeevIdentityResult.Failed(string.Format(L("Identity.DuplicateUserName"), userName));
            }

            user = (await FindByEmailAsync(emailAddress));
            if (user != null && user.Id != expectedUserId)
            {
                return GeevIdentityResult.Failed(string.Format(L("Identity.DuplicateEmail"), emailAddress));
            }
        ...
        private async Task CheckMaxUserOrganizationUnitMembershipCountAsync(int? tenantId, int requestedCount)
        {
            ...
            {
                throw new GeevException(string.Format("Can not set more than {0} organization unit for a user!", maxCount));
            }
        }
        ...
        public virtual Task<List<TUser>> GetUsersInOrganizationUnit(OrganizationUnit organizationUnit, bool includeChildren = false)
        {
            ...
            {
                var query = from uou in _userOrganizationUnitRepository.GetAll()
                            join user in GeevStore.Users on uou.UserId equals user.Id
        ...
        public virtual void RegisterTwoFactorProviders(int? tenantId)
        {
            ...
            if (!IsTrue(GeevZeroSettingNames.UserManagement.TwoFactorLogin.IsEnabled, tenantId))
            {
                return;
            }

            if (EmailService != null &&
                IsTrue(GeevZeroSettingNames.UserManagement.TwoFactorLogin.IsEmailProviderEnabled, tenantId))
            {
            ...
            if (SmsService != null &&
                IsTrue(GeevZeroSettingNames.UserManagement.TwoFactorLogin.IsSmsProviderEnabled, tenantId))
            {
                RegisterTwoFactorProvider(
                    L("Sms"),
        ...
        {
            UserLockoutEnabledByDefault = IsTrue(GeevZeroSettingNames.UserManagement.UserLockOut.IsEnabled, tenantId);
            DefaultAccountLockoutTimeSpan = TimeSpan.FromSeconds(GetSettingValue<int>(GeevZeroSettingNames.UserManagement.UserLockOut.DefaultAccountLockoutSeconds, tenantId));
            MaxFailedAccessAttemptsBeforeLockout = GetSettingValue<int>(GeevZeroSettingNames.UserManagement.UserLockOut.MaxFailedAccessAttemptsBeforeLockout, tenantId);
        }
        ...
        {
            return GeevStore.GetUserNameFromDatabaseAsync(userId);
        }

...
namespace Geev.EntityFrameworkCore
{
    ...
    /// </summary>
    public abstract class GeevDbContext : DbContext, ITransientDependency
    {
        /// <summary>
        ...
        protected virtual bool IsSoftDeleteFilterEnabled => CurrentUnitOfWorkProvider?.Current?.IsFilterEnabled(GeevDataFilters.SoftDelete) == true;

        protected virtual bool IsMayHaveTenantFilterEnabled => CurrentUnitOfWorkProvider?.Current?.IsFilterEnabled(GeevDataFilters.MayHaveTenant) == true;

        protected virtual bool IsMustHaveTenantFilterEnabled => CurrentTenantId != null && CurrentUnitOfWorkProvider?.Current?.IsFilterEnabled(GeevDataFilters.MustHaveTenant) == true;

        private static MethodInfo ConfigureGlobalFiltersMethodInfo = typeof(GeevDbContext).GetMethod(nameof(ConfigureGlobalFilters), BindingFlags.Instance | BindingFlags.NonPublic);

        /// <summary>
        /// Constructor.
        /// </summary>
        protected GeevDbContext(DbContextOptions options)
            : base(options)
        {
            InitializeDbContext();
        }
        ...
        public override int SaveChanges()
        {
            ...
            {
                throw new GeevDbConcurrencyException(ex.Message, ex);
            }
        }
        ...
        protected virtual void ApplyGeevConcepts(EntityEntry entry, long? userId, EntityChangeReport changeReport)
        {
            switch (entry.State)
            {
        ...
        protected virtual void ApplyGeevConceptsForAddedEntity(EntityEntry entry, long? userId, EntityChangeReport changeReport)
        {
            CheckAndSetId(entry);
        ...
        protected virtual void ApplyGeevConceptsForModifiedEntity(EntityEntry entry, long? userId, EntityChangeReport changeReport)
        {
        ...
        protected virtual void ApplyGeevConceptsForDeletedEntity(EntityEntry entry, long? userId, EntityChangeReport changeReport)
        {
        ...
        {
            EntityAuditingHelper.SetCreationAuditProperties(MultiTenancyConfig, entityAsObj, GeevSession.TenantId, userId);
        }

        protected virtual void SetModificationAuditProperties(object entityAsObj, long? userId)
        {
            EntityAuditingHelper.SetModificationAuditProperties(MultiTenancyConfig, entityAsObj, GeevSession.TenantId, userId);
        }

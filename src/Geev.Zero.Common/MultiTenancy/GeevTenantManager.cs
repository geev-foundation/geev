...
namespace Geev.MultiTenancy
{
    /// <summary>
    /// Tenant manager.
    /// Implements domain logic for <see cref="GeevTenant{TUser}"/>.
    /// </summary>
    ...
    public class GeevTenantManager<TTenant, TUser> : IDomainService,
    ...
    {
        public GeevEditionManager EditionManager { get; set; }
        ...
        private readonly IGeevZeroFeatureValueStore _featureValueStore;

        public GeevTenantManager(
            IRepository<TTenant> tenantRepository, 
            IRepository<TenantFeatureSetting, long> tenantFeatureRepository,
            GeevEditionManager editionManager,
        ...
        public virtual async Task<TTenant> GetByIdAsync(int id)
        {
            ...
            if (tenant == null)
            {
                throw new GeevException("There is no tenant with id: " + id);
            }
        ...
        {
            if (!Regex.IsMatch(tenancyName, GeevTenant<TUser>.TenancyNameRegex))
            {

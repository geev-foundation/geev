...
namespace Geev.Zero.Ldap.Authentication
{
    /// <summary>
        ...
        where TUser : GeevUserBase, new()
    {
        ...
        private readonly IGeevZeroLdapModuleConfig _ldapModuleConfig;

        protected LdapAuthenticationSource(ILdapSettings settings, IGeevZeroLdapModuleConfig ldapModuleConfig)
        {
            _settings = settings;
        ...
        public override async Task<TUser> CreateUserAsync(string userNameOrEmailAddress, TTenant tenant)
        {
            ...
            using (var principalContext = await CreatePrincipalContext(tenant, user))
            {
                ...
                {
                    throw new GeevException("Unknown LDAP user: " + userNameOrEmailAddress);
                }
        ...
        public override async Task UpdateUserAsync(TUser user, TTenant tenant)
        {
            ...
            using (var principalContext = await CreatePrincipalContext(tenant, user))
            {
                ...
                if (userPrincipal == null)
                {
                    throw new GeevException("Unknown LDAP user: " + user.UserName);
                }
        ...
        protected virtual async Task CheckIsEnabled(TTenant tenant)
        {
            ...
            {
                throw new GeevException("Ldap Authentication module is disabled globally!");                
            }

            var tenantId = tenant?.Id;
            if (!await _settings.GetIsEnabled(tenantId))
            {
                throw new GeevException("Ldap Authentication is disabled for given tenant (id:" + tenantId + ")! You can enable it by setting '" + LdapSettingNames.IsEnabled + "' to true");
            }
        }

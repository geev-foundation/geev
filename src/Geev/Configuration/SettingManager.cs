...
namespace Geev.Configuration
{
    ...
    public class SettingManager : ISettingManager, ISingletonDependency
    {
        ...
        {
            return GetSettingValueInternalAsync(name, GeevSession.TenantId, GeevSession.UserId);
        }
        ...
        public async Task<IReadOnlyList<ISettingValue>> GetAllSettingValuesAsync(SettingScopes scopes)
        {
            ...
            if (scopes.HasFlag(SettingScopes.Application))
            {
                foreach (var settingValue in await GetAllSettingValuesForApplicationAsync())
                {
                    ...
                    if (!setting.IsInherited &&
                        ((setting.Scopes.HasFlag(SettingScopes.Tenant) && GeevSession.TenantId.HasValue) || (setting.Scopes.HasFlag(SettingScopes.User) && GeevSession.UserId.HasValue)))
                    {
                        continue;
                    }
            ...
            if (scopes.HasFlag(SettingScopes.Tenant) && GeevSession.TenantId.HasValue)
            {
                foreach (var settingValue in await GetAllSettingValuesForTenantAsync(GeevSession.TenantId.Value))
                {
                    var setting = settingDefinitions.GetOrDefault(settingValue.Name);
                    ...
                    if (!setting.IsInherited &&
                        (setting.Scopes.HasFlag(SettingScopes.User) && GeevSession.UserId.HasValue))
                    {
                        continue;
                    }
            ...
            if (scopes.HasFlag(SettingScopes.User) && GeevSession.UserId.HasValue)
            {
                foreach (var settingValue in await GetAllSettingValuesForUserAsync(GeevSession.ToUserIdentifier()))
                {
                    var setting = settingDefinitions.GetOrDefault(settingValue.Name);
        ...
        public async Task<IReadOnlyList<ISettingValue>> GetAllSettingValuesForApplicationAsync()
        {
            ...
            {
                return (await GetReadOnlyTenantSettings(GeevSession.GetTenantId())).Values
        ...
        {
            return GetAllSettingValuesForUserAsync(new UserIdentifier(GeevSession.TenantId, userId));
        }
        ...
        public virtual async Task ChangeSettingForApplicationAsync(string name, string value)
        {
            ...
            else
            {
                ...
                await InsertOrUpdateOrDeleteSettingValueAsync(name, value, GeevSession.GetTenantId(), null);
                await _tenantSettingCache.RemoveAsync(GeevSession.GetTenantId());
            }
        ...
        {
            return ChangeSettingForUserAsync(new UserIdentifier(GeevSession.TenantId, userId), name, value);
        }
        ...
        private async Task<SettingInfo> GetSettingValueForApplicationOrNullAsync(string name)
        {
            ...
            return (await GetReadOnlyTenantSettings(GeevSession.GetTenantId())).GetOrDefault(name);
        }

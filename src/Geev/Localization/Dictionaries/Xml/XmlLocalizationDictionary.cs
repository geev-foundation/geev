...
using Geev.Xml.Extensions;

namespace Geev.Localization.Dictionaries.Xml
{
    /// <summary>
    ...
    public class XmlLocalizationDictionary : LocalizationDictionary
    {
        ...
        public static XmlLocalizationDictionary BuildFomFile(string filePath)
        {
            ...
            {
                throw new GeevException("Invalid localization file format! " + filePath, ex);
            }
        }
        ...
        public static XmlLocalizationDictionary BuildFomXmlString(string xmlString)
        {
            ...
            {
                throw new GeevException("A Localization Xml must include localizationDictionary as root node.");
            }

            var cultureName = localizationDictionaryNode[0].GetAttributeValueOrNull("culture");
            if (string.IsNullOrEmpty(cultureName))
            {
                throw new GeevException("culture is not defined in language XML file!");
            }
            ...
            if (textNodes != null)
            {
                foreach (XmlNode node in textNodes)
                {
                    ...
                    {
                        throw new GeevException("name attribute of a text is empty in given xml string.");
                    }
            ...
            {
                throw new GeevException("A dictionary can not contain same key twice. There are some duplicated names: " + dublicateNames.JoinAsString(", "));
            }

...
{
    public abstract class GeevSignInManager<TTenant, TRole, TUser> : ITransientDependency
           where TTenant : GeevTenant<TUser>
           where TRole : GeevRole<TUser>, new()
           where TUser : GeevUser<TUser>
    {
        public GeevUserManager<TRole, TUser> UserManager { get; set; }
        ...
        private readonly IGeevZeroAspNetCoreConfiguration _configuration;
        ...
        protected GeevSignInManager(
            ...
            IGeevZeroAspNetCoreConfiguration configuration)
        {
        ...
        [UnitOfWork]
        public virtual async Task<SignInStatus> SignInOrTwoFactorAsync(GeevLoginResult<TTenant, TUser> loginResult, bool isPersistent, bool? rememberBrowser = null)
        {
        ...
        /// <param name="rememberBrowser">Remember user's browser (and not use two factor auth again) or not.</param>
        [UnitOfWork]
        public virtual async Task SignInAsync(GeevLoginResult<TTenant, TUser> loginResult, bool isPersistent, bool? rememberBrowser = null)
        {
            if (loginResult.Result != GeevLoginResultType.Success)
            {
                throw new ArgumentException("loginResult.Result should be success in order to sign in!");
        ...
        private static ClaimsPrincipal CreateIdentityForTwoFactor(TUser user, string authType)
        {
            ...
            {
                claimsIdentity.AddClaim(new Claim(GeevClaimTypes.TenantId, user.TenantId.Value.ToString()));
            }
        ...
        public virtual async Task SignInAsync(TUser user, bool isPersistent, bool? rememberBrowser = null)
        {
            using (_unitOfWorkManager.Current.SetTenantId(user.TenantId))
            {
                ...
                if (rememberBrowser == null)
                {
                    rememberBrowser = IsTrue(GeevZeroSettingNames.UserManagement.TwoFactorLogin.IsRememberBrowserEnabled, user.TenantId);
                }
        ...
        {
            var authenticateResult = await AuthenticationManager.AuthenticateAsync(_configuration.TwoFactorAuthenticationScheme);
            return GeevZeroClaimsIdentityHelper.GetTenantId(authenticateResult?.Identity);
        }
        ...
        public async Task<bool> TwoFactorBrowserRememberedAsync(string userId, int? tenantId)
        {
            ...
            if (GeevZeroClaimsIdentityHelper.GetTenantId(result.Identity) != tenantId)
            {
                return false;
            }

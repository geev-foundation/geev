        ...
        public void Should_Disable_And_Enable_Filters_For_SoftDelete()
        {
            ...
            using (var ouw = uowManager.Begin())
            {
                ...
                using (uowManager.Current.DisableFilter(GeevDataFilters.SoftDelete))
                {
                    ...
                    using (uowManager.Current.EnableFilter(GeevDataFilters.SoftDelete)) //re-enabling filter
                    {
                        _personRepository.GetAllList().Any(p => p.Name == "emre").ShouldBe(false); //not getting deleted people

                        using (uowManager.Current.EnableFilter(GeevDataFilters.SoftDelete)) //enabling filter has no effect since it's already enabed
                        {
                            _personRepository.GetAllList().Any(p => p.Name == "emre").ShouldBe(false); //not getting deleted people
                        }

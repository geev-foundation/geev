...
using Geev.EntityFrameworkCore.EFPlus;
using Geev.ZeroCore.SampleApp.Core;
using Shouldly;
using Xunit;

namespace Geev.Zero.EFPlus
{
    public class GeevEntityFrameworkCoreEfPlusExtensions_Tests : GeevZeroTestBase
    {
        private readonly IRepository<Role> _roleRepository;
        private readonly IUnitOfWorkManager _unitOfWorkManager;

        public GeevEntityFrameworkCoreEfPlusExtensions_Tests()
        {
        ...
        public async Task Should_Delete_All_Roles_When_MayHaveTenant_Filter_Is_Disabled_When_Using_BatchDelete()
        {
            using (var uow = _unitOfWorkManager.Begin())
            {
                using (_unitOfWorkManager.Current.DisableFilter(GeevDataFilters.MayHaveTenant))
                {
                    await _roleRepository.BatchDeleteAsync(r => r.Id > 0);

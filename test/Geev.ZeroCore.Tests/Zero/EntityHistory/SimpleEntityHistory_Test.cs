...
namespace Geev.Zero.EntityHistory
{
    public class SimpleEntityHistory_Test : GeevZeroTestBase
    {
        ...
        public void Should_Write_History_For_Tracked_Entities_Create()
        {
            ...
            _entityHistoryStore.Received().SaveAsync(Arg.Is<EntityChangeSet>(
                s => s.EntityChanges.Count == 2 &&
                     ...
                     s.ImpersonatorTenantId == GeevSession.ImpersonatorTenantId &&
                     s.ImpersonatorUserId == GeevSession.ImpersonatorUserId &&
                     s.TenantId == GeevSession.TenantId &&
                     s.UserId == GeevSession.UserId
            ));
        }

...
{
    public class Validation_Tests : GeevIntegratedTestBase<GeevKernelModule>
    {
        ...
        {
            Assert.Throws<GeevValidationException>(() => _myAppService.MyMethod(new MyMethodInput())); //MyStringValue is not supplied!
            Assert.Throws<GeevValidationException>(() => _myAppService.MyMethod(new MyMethodInput { MyStringValue = "a" })); //MyStringValue's min length should be 3!
        }
        ...
        {
            Assert.Throws<GeevValidationException>(() =>

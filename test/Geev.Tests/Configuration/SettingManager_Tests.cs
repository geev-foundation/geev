...
using Geev.Runtime.Caching.Memory;
using Geev.Runtime.Remoting;
using Geev.Runtime.Session;
using Geev.TestBase.Runtime.Session;
using Geev.Tests.MultiTenancy;
using JetBrains.Annotations;
using NSubstitute;
using Shouldly;
using Xunit;

namespace Geev.Tests.Configuration
{
    public class SettingManager_Tests : TestBaseWithLocalIocManager
    {
        ...
        private SettingManager CreateSettingManager(bool multiTenancyIsEnabled = true)
        {
            return new SettingManager(
                ...
                new GeevMemoryCacheManager(
                    LocalIocManager,
                    new CachingConfiguration(Substitute.For<IGeevStartupConfiguration>())
                    ),
        ...
        {
            var session = CreateTestGeevSession();

            var settingManager = CreateSettingManager();
            settingManager.SettingStore = new MemorySettingStore();
            settingManager.GeevSession = session;
        ...
        {
            // Arrange
            var session = CreateTestGeevSession(multiTenancyIsEnabled: false);
        ...
        private static TestGeevSession CreateTestGeevSession(bool multiTenancyIsEnabled = true)
        {
